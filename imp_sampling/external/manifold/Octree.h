...
public:
	...
	void traverseBreadth(vector<pair<Vector3d, Vector3d>> &cells, int &count)
	{
		// cout << "level: " << level << endl;
		queue<Octree *> q;
		q.push(this);
		while (!q.empty())
		{
			...
			q.pop();
			count++;
			cells.push_back(make_pair(
				Vector3d(cur->min_corner[0], cur->min_corner[1], cur->min_corner[2]),
				Vector3d(cur->length[0], cur->length[1], cur->length[2])));
			if (cur->level > 0)
			{
				for (int i = 0; i < 8; ++i)
				{
	...
	void traverseOccupiedCells(vector<pair<Vector3d, Vector3d>> &cells, int &count, vector<Vector4i> &tree_id, int loc_id, int parent_id)
	{
		// // cout << "level: " << level << endl;
		// if (level == 0)
		// {
		...
		count++;
		if (occupied)
		{	tree_id.push_back(Vector4i(parent_id, level, loc_id, cells.size()));
			cells.push_back(make_pair(Vector3d(min_corner[0], min_corner[1], min_corner[2]), Vector3d(length[0], length[1], length[2])));
			// cout << "There is no children!" << endl;
			// cout << "min corner: " << min_corner[0] << " " << min_corner[1] << " " << min_corner[2] << endl;
			// cout << "length: " << length[0] << " " << length[1] << " " << length[2] << endl;
		...
		{
			tree_id.push_back(Vector4i(parent_id, level, loc_id, -1));
		}
		if (level > 0)
		{
			// cout << "Checking children!" << endl;
			for (int i = 0; i < 8; ++i)
			{
				children[i]->traverseOccupiedCells(cells, count, tree_id, i, now_id);
			}
		}
	}
